...
function cnpj(event: FormEvent<HTMLInputElement> | string) {
  ...
  function apply(value: string) {
    if (!/^(\d{2}).(\d{3}).(\d{3})\/(\d{4})-(\d{2})$/.exec(value)) {
...
function phone(event: FormEvent<HTMLInputElement> | string) {
  ...
  function apply(value: string) {
    if (!/^\((\d{2})\)[ ](\d{5})-(\d{4})/.exec(value)) {
      value = value.replace(/\D/g, "");
      value = value.replace(/(\d{2})(\d)/, "($1) $2");
      value = value.replace(/(\d{4,5})(\d{4})\d*$/, "$1-$2");
    }
...
function currency(event: FormEvent<HTMLInputElement> | string | number) {
  function apply(value: string | number) {
    if (!value) return "";
